...
test('ClickOutside event should only fire on click outside of target node.', async ({
	mount,
	page
}) => {
...
test('ClickOutside callback should only fire on click outside of target node.', async ({
	mount,
	page
}) => {
	await mount(Component);
