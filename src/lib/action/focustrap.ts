 ...
 */
 export const focustrap = (node: HTMLElement) => {
    ...
    const reFocus = (e: FocusEvent) => { (!focusable.includes(e.relatedTarget as HTMLElement)) && (e.target as HTMLElement).focus() }
    
    const handleKeyDown = (e: KeyboardEvent) => {
        if (e.key === "Tab") {
            e.shiftKey
            ? (e.preventDefault(), focusable[(focusable.indexOf(e.target as HTMLElement) - 1 + focusable.length) % focusable.length].focus())
            : (e.preventDefault(), focusable[(focusable.indexOf(e.target as HTMLElement) + 1) % focusable.length].focus())
        }
        if (e.key === "Escape") {
            cancel()
        }
